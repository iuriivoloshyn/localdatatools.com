              ...
              <div className="space-y-0.5">
                {items.map(({ id, label }) => (
                  <button
                    key={id}
                    onClick={() => navigate(id)}
                    className={`block w-full text-left px-3 py-1.5 rounded-lg text-[13px] transition-colors ${activePage === id ? 'text-indigo-400 bg-indigo-500/10 font-medium' : 'text-gray-500 hover:text-gray-300'}`}
                  >
                    {label}
                  </button>
                ))}
