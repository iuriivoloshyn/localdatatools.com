...
const LARGE_FILE_MAX = 1024 * 1024 * 1024; // 1GB absolute max
...
// Auto-routes to encrypted R2 pipeline for files > 50MB
csvRoutes.post('/merge', async (c) => {
  ...
  const hasLargeFile = files.some(f => f.size > MAX_FILE_SIZE);
  ...
  // Large files: encrypt and store in R2
  if (hasLargeFile) {
...
csvRoutes.post('/diff', async (c) => {
  ...
  if (fileA.size > LARGE_FILE_MAX || fileB.size > LARGE_FILE_MAX) {
...
csvRoutes.post('/anonymize', async (c) => {
  ...
  const isLargeAnonymize = file.size > MAX_FILE_SIZE;
  if (isLargeAnonymize && !isR2Configured()) {
    return c.json({ error: 'Large file storage not configured.' }, 503);
  }
  if (!configParam) {
  ...
  // Large files: encrypt and store in R2
  if (isLargeAnonymize) {
    const { jobId, fileKey } = await storeEncrypted(zipBuffer, {
      contentType: 'application/zip',
      fileName: `${baseName}_anonymized.zip`,
    });
    return c.json({
      jobId,
      fileKey,
...
csvRoutes.post('/deanonymize', async (c) => {
  ...
  const isLargeDeanon = file.size > MAX_FILE_SIZE;
  if (isLargeDeanon && !isR2Configured()) {
    return c.json({ error: 'Large file storage not configured.' }, 503);
  }
  ...
  if (isLargeDeanon) {
    ...
    const { jobId, fileKey } = await storeEncrypted(resultBuffer, {
      contentType: 'text/csv',
      fileName: `${baseName}_restored.csv`,
    });
    return c.json({
      jobId,
      fileKey,
...
csvRoutes.post('/join', async (c) => {
  ...
  const hasLargeFile = fileA.size > MAX_FILE_SIZE || fileB.size > MAX_FILE_SIZE;
