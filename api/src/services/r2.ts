...
export async function storeEncrypted(data: Buffer, options?: { contentType?: string; fileName?: string }): Promise<StoredFile> {
  ...
  if (options?.fileName) metadata['file-name'] = options.fileName;
...
export async function fetchDecrypted(jobId: string, fileKey: string): Promise<DecryptedFile> {
  const client = getClient();
  ...
  const data = decryptBuffer(encrypted, fileKey);
  const contentType = response.Metadata?.['content-type'] || 'text/csv';
  const fileName = response.Metadata?.['file-name'] || `${jobId}.csv`;
